...
import {
  ...
  createWabaGatewayPhoneWebhook,
  getKapsoApiKey,
  listKapsoPhoneWebhooks,
  listWabaGatewayPhoneWebhooks,
  resolveWabaGatewayPhoneRecordId,
...
async function safeSendWhatsApp(
  ...
  try {
    await sendWhatsAppTextForProvider(provider, phoneNumberId, to, text);
    return true;
  } catch (err) {
...
function requireProForCustomerMode(profile: any, connectionType: string) {
  ...
  return new Response(
    JSON.stringify({
      error:
        "Customer-facing WhatsApp is a Pro feature. Internal WhatsApp setup is available on Free.",
    }),
...
function gatewayWebhookSecretConfigured(): boolean {
  return !!(Deno.env.get("WABA_GATEWAY_WEBHOOK_SECRET") ||
    KAPSO_WEBHOOK_SECRET);
}

function gatewayConfigured(): boolean {
  return !!Deno.env.get("WABA_GATEWAY_API_KEY") &&
    gatewayWebhookSecretConfigured();
}

function gatewaySetupUrl(userId: string, connectionType: string) {
  const base = Deno.env.get("WABA_GATEWAY_SETUP_URL") ||
    `${
      (Deno.env.get("WABA_GATEWAY_BASE_URL") || "https://waba.hoursback.xyz")
        .replace(/\/+$/, "")
    }/connect`;
  const webhookSecret = Deno.env.get("WABA_GATEWAY_WEBHOOK_SECRET") ||
    KAPSO_WEBHOOK_SECRET;
...
function extractKapsoSetupLinkExpiry(setupLink: any) {
  return firstString(
    ...
    setupLink?.setup_link?.expires_at,
    setupLink?.data?.expires_at,
  );
}
...
function extractKapsoWebhookRows(response: any) {
  ...
  if (Array.isArray(response?.whatsapp_webhooks)) {
    return response.whatsapp_webhooks;
  }
  return [];
}

function extractWabaGatewayWebhookRows(response: any) {
  if (Array.isArray(response)) return response;
  if (Array.isArray(response?.data)) return response.data;
...
async function registerKapsoWebhookForNumber(
  ...
  if (!KAPSO_WEBHOOK_SECRET) {
    throw new Error(
      "KAPSO_WEBHOOK_SECRET is not configured in Supabase secrets",
    );
  }
  ...
  const existingWebhookId = existingWebhook
    ? extractKapsoWebhookId(existingWebhook)
    : null;
...
async function registerWabaGatewayWebhookForNumber(
  supabase: any,
  userId: string,
  connectionType: string,
  phoneNumberId: string,
) {
  const secret = Deno.env.get("WABA_GATEWAY_WEBHOOK_SECRET") ||
    KAPSO_WEBHOOK_SECRET;
  if (!Deno.env.get("WABA_GATEWAY_API_KEY")) {
    throw new Error(
      "WABA_GATEWAY_API_KEY is not configured in Supabase secrets",
    );
  }
  if (!secret) {
    throw new Error(
      "WABA_GATEWAY_WEBHOOK_SECRET is not configured in Supabase secrets",
    );
  }

  const url = wabaGatewayWebhookUrl(userId, connectionType);
  const gatewayPhoneRecordId = await resolveWabaGatewayPhoneRecordId(
    phoneNumberId,
  );
  const response = await listWabaGatewayPhoneWebhooks(gatewayPhoneRecordId);
  const existing = extractWabaGatewayWebhookRows(response).find((item: any) => (
    firstString(item?.url, item?.webhook?.url) === url &&
    item?.active !== false && item?.webhook?.active !== false
  ));
...
async function markGlobalZernioWebhookActive(
  ...
  const webhook = webhookSettings?.webhook || webhookSettings?.data?.webhook ||
  ...
  await logAnalyticsEvent(
    supabase,
    userId,
    "whatsapp_zernio_routing_registered",
    {
      connection_type: connectionType,
      phone_number_id: phoneNumberId,
      webhook_url: url,
...
async function fetchConnectionForUser(
  supabase: any,
  userId: string,
  connectionType: string,
) {
...
function orderItemsSummary(items: any[]) {
  ...
  return items.map((item) => `${item.qty ? `${item.qty} x ` : ""}${item.name}`)
    .join(", ");
}
...
async function syncVerifiedOrderToSalesLog(supabase: any, order: any) {
  ...
  const paidAt = order.paid_at || order.payment_verified_at ||
  ...
  const { error } = await supabase.from("bot_entries").insert({
    user_id: order.user_id,
    chat_id: 0,
    triggered_by: order.customer_name || order.customer_phone ||
      "WhatsApp customer",
    role: "customer",
    raw_text: order.raw_text ||
      `Verified WhatsApp request ${order.order_code || order.id}`,
    entry_type: "sale",
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "status") {
      ...
      const primary = connections.find((item: any) =>
        item.connection_type === "internal"
      ) || connections[0] || null;

      const setupProvider = getWhatsAppSetupProvider();
      const apiConfigured = setupProvider === "zernio"
        ? !!getZernioApiKey()
        : setupProvider === "waba_gateway"
        ? gatewayConfigured()
        : !!getKapsoApiKey();
      const setupLinkConfigured = setupProvider === "zernio"
        ? !!getZernioApiKey()
        : setupProvider === "waba_gateway"
        ? gatewayWebhookSecretConfigured()
        : true;
      return new Response(
        JSON.stringify({
          ...
          webhook_secret_configured: webhookSecretConfiguredForProvider(
            setupProvider,
          ),
    ...
    if (action === "disconnect") {
      const connectionType = body.connection_type === "customer"
        ? "customer"
        : body.connection_type === "internal"
        ? "internal"
        : null;
      let query = supabase.from("kapso_connections").delete().eq(
        "user_id",
        user.id,
      );
    ...
    if (action === "register_webhook") {
      ...
      const connection = await fetchConnectionForUser(
        supabase,
        user.id,
        connectionType,
      );
      if (!connection?.phone_number_id) {
        return new Response(
          JSON.stringify({
            error:
              "Connect a WhatsApp number before registering message routing.",
          }),
      ...
      try {
        ...
        if (connection.whatsapp_provider === "zernio") {
          ...
          webhook = await markGlobalZernioWebhookActive(
            supabase,
            user.id,
            connectionType,
          ...
          webhook = await registerWabaGatewayWebhookForNumber(
            supabase,
            user.id,
            connectionType,
            connection.phone_number_id,
          ...
          webhook = await registerKapsoWebhookForNumber(
            supabase,
            user.id,
            connectionType,
            connection.phone_number_id,
        ...
        const refreshed = await fetchConnectionForUser(
          supabase,
          user.id,
          connectionType,
        );
        ...
        await supabase
          ...
          .update({
            status: "connected",
            kapso_webhook_registered_at: null,
            kapso_webhook_error: message,
            webhook_secret_set: webhookSecretConfiguredForProvider(
              connection.whatsapp_provider || getWhatsAppSetupProvider(),
            ),
        ...
        await logAnalyticsEvent(
          supabase,
          user.id,
          "whatsapp_webhook_registration_failed",
          {
            connection_type: connectionType,
        ...
        return new Response(
          JSON.stringify({ success: false, connection, error: message }),
    ...
    if (action === "customer_settings") {
      ...
      const paymentInstructions = String(body.payment_instructions || "")
        .trim();
      const ownerNotificationNumber = String(
        body.owner_notification_number || "",
      ).trim();
      ...
      const escalationInstructions = String(body.escalation_instructions || "")
        .trim();
      ...
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          status: existing?.kapso_webhook_registered_at
            ? "webhook_active"
            : existing?.phone_number_id
            ? "connected"
            : "settings_saved",
          webhook_secret_set: webhookSecretConfiguredForProvider(
            existing?.whatsapp_provider || getWhatsAppSetupProvider(),
          ),
    ...
    if (action === "verify_order_payment") {
      ...
      if (!order.receipt_storage_path) {
        return new Response(
          JSON.stringify({
            error:
              "Receipt file is not available. Ask the customer to resend the receipt before confirming payment.",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
      if (
        order.status !== "confirmed" || order.payment_status !== "receipt_sent"
      ) {
        return new Response(
          JSON.stringify({
            error:
              "Only confirmed requests with a pending receipt can be marked paid.",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
      ...
      if (connection?.phone_number_id && order.customer_phone) {
        ...
        const message = [
          ...
          `Request: ${items}`,
          deliveryNote ||
          "We are processing your request now and will update you if anything changes.",
        ].join("\n");
        messageSent = await safeSendWhatsApp(
          connection.whatsapp_provider,
          connection.phone_number_id,
      ...
      const salesLogSynced = await syncVerifiedOrderToSalesLog(
        supabase,
        updatedOrder,
      );
    ...
    if (action === "reject_order_payment") {
      ...
      if (
        ...
        return new Response(
          JSON.stringify({
            error:
              "Only confirmed requests with a pending receipt can be rejected.",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
      ...
      const ownerNotes = [
        existingNotes,
        `Payment receipt rejected at ${now}; customer asked to resend proof.`,
      ]
        .filter(Boolean)
        .join("\n");
      const { data: updatedOrder, error: updateError } = await supabase
    ...
    if (action === "cancel_order") {
      ...
      if (order.status === "cancelled" || order.status === "fulfilled") {
        return new Response(
          JSON.stringify({
            error: "Cancelled or fulfilled requests cannot be cancelled again.",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
      ...
      let messageSent = false;
      if (
        notifyCustomer && connection?.phone_number_id && order.customer_phone
      ) {
        const items = Array.isArray(order.items)
          ? orderItemsSummary(order.items)
          : "your request";
      ...
      await logOrderAudit(supabase, updatedOrder, "cancelled_by_owner", {
        reason,
      }, messageSent);
    ...
    if (action === "mark_stale_unpaid_orders") {
      ...
      const rows = (orders || []).filter((order: any) =>
        String(order.payment_method || "").toLowerCase() !== "cash on pickup"
      );
      if (!rows.length) {
    ...
    if (action === "update_order_review") {
      ...
      const deliveryFee =
        body.delivery_fee_amount === "" || body.delivery_fee_amount == null
          ? null
          : Number(body.delivery_fee_amount);
      const expectedTotal =
        body.expected_total_amount === "" || body.expected_total_amount == null
          ? null
          : Number(body.expected_total_amount);
      const adjustedTotal = body.owner_adjusted_total_amount === "" ||
          body.owner_adjusted_total_amount == null
        ? null
        : Number(body.owner_adjusted_total_amount);
      const ownerNotes = String(body.owner_notes || "").trim();
      if (
        [deliveryFee, expectedTotal, adjustedTotal].some((value) =>
          value !== null && (!Number.isFinite(value) || value < 0)
        )
      ) {
        return new Response(
          JSON.stringify({
            error: "Amounts must be valid non-negative numbers.",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
    ...
    if (action === "update_order_fulfillment") {
      ...
      if (
        ...
        return new Response(
          JSON.stringify({ error: "Invalid fulfillment_status" }),
          { status: 400, headers: corsHeaders },
        );
      }
      ...
      if (order.status === "cancelled" || order.status === "fulfilled") {
        return new Response(
          JSON.stringify({
            error: "Cancelled or fulfilled requests cannot be updated.",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
    ...
    if (action === "manual_connect") {
      ...
      const { data, error } = await supabase
        ...
        .upsert({
          ...
          customer_menu: connectionType === "customer"
            ? customerMenu || null
            : null,
          payment_instructions: connectionType === "customer"
            ? paymentInstructions || null
            : null,
          owner_notification_number: connectionType === "customer"
            ? ownerNotificationNumber || null
            : null,
          business_type: connectionType === "customer"
            ? businessType || null
            : null,
          operating_hours: connectionType === "customer"
            ? operatingHours || null
            : null,
          fulfillment_rules: connectionType === "customer"
            ? fulfillmentRules || null
            : null,
          escalation_instructions: connectionType === "customer"
    ...
    if (action === "generate_setup_link") {
      ...
      if (getWhatsAppSetupProvider() === "waba_gateway") {
        ...
        await logAnalyticsEvent(
          supabase,
          user.id,
          "whatsapp_gateway_setup_link_created",
          {
            connection_type: connectionType,
            provider: "waba_gateway",
          },
      ...
      // ── Zernio: Meta Embedded Sign-Up flow ──────────────────────────────
      if (getWhatsAppSetupProvider() === "zernio") {
        if (!getZernioApiKey()) {
          return new Response(
            JSON.stringify({
        ...
        const callbackUrl =
          `${appOrigin}/whatsapp/callback?mode=${connectionType}&provider=zernio`;
        const profileId = await getOrCreateZernioProfile(
          `Hoursback ${user.email || user.id} ${connectionType}`,
        );
        const { authUrl: setupLinkUrl, state: setupLinkId } =
          await createZernioWhatsAppConnectUrl(profileId, callbackUrl);
        ...
        await logAnalyticsEvent(
          supabase,
          user.id,
          "whatsapp_zernio_setup_link_created",
          {
            connection_type: connectionType,
            provider: "zernio",
      ...
      if (!getKapsoApiKey()) {
        return new Response(
          JSON.stringify({
            error: "KAPSO_API_KEY is not configured in Supabase secrets",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
      ...
      const externalCustomerId = existing?.external_customer_id ||
        `hoursback:${user.id}:${connectionType}`;
      ...
      const callbackBase =
        `${appOrigin}/whatsapp/callback?mode=${connectionType}`;
      let setup;
      ...
      try {
        ...
        console.warn(
          "Kapso enhanced setup link failed, falling back to basic setup link:",
          err,
        );
      ...
      return new Response(
        JSON.stringify({ success: true, connection: data, provider: "kapso" }),
        { headers: corsHeaders },
      );
    }
    ...
    if (action === "finalize_setup") {
      ...
      const setupLinkId = String(body.setup_link_id || body.setupLinkId || "")
        .trim();
      const setupProvider = getWhatsAppSetupProvider();
      ...
      if (!phoneNumberId) {
        return new Response(
          JSON.stringify({
            error: "phone_number_id is required to finish WhatsApp setup",
          }),
          { status: 400, headers: corsHeaders },
        );
      }
